...
class LevelCrossingCatalogTest < ActiveSupport::TestCase
  setup do
    ...
    @output = Rails.root.join("tmp/test_level_crossings.json")
  end
  ...
  test "writes estimated mid-corridor crossing features" do
    count = Geojson::LevelCrossingCatalog.refresh!(output: @output)
    assert count >= 1

    data = JSON.parse(@output.read)
    assert_equal "FeatureCollection", data["type"]
    feature = data["features"].find { |row| row.dig("properties", "route_id") == "test_crossing_line" }
    assert feature
