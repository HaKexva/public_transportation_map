...
module Transit
  # Seeds published timetables for all `other` system routes (excluding Alishan,
  # which has its own dedicated seeder that this class also invokes, and sugar
  # railways which use SugarRailwayScheduleSeeder).
  class OtherTransitScheduleSeeder
    ...
    def seed!
      ...
      dataset = ScheduleDataset.find_or_create_by!(name: OtherTransitScheduleCatalog::DATASET_NAME) do |record|
        record.source = "manual"
        record.valid_from = Date.new(2026, 1, 1)
        record.notes = OtherTransitScheduleCatalog::SOURCE_NOTE
      end
      dataset.update!(
        notes: OtherTransitScheduleCatalog::SOURCE_NOTE,
      ...
      OtherTransitScheduleCatalog::ROUTES.each do |route_id, spec|
        route = TransitRoute.find_by(system_id: "other", route_id: route_id)
        unless route
      ...
      Transit::AlishanScheduleSeeder.seed!
    ...
    def annotated_notes(notes, route_id)
      source = OtherTransitScheduleCatalog::ROUTES.dig(route_id, :source)
